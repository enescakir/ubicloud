...
RSpec.describe GithubInstallationSpillOption do
  subject(:spill_option) {
    described_class.create(x64_vcpus_limit: 300, arm64_vcpus_limit: 300) { it.id = installation.id }
  }
  ...
  it "is enabled and defaults spill_ratio and allocated vcpus to zero" do
    expect(spill_option.enabled).to be(true)
    expect(spill_option.spill_ratio).to eq(0)
    expect(spill_option.x64_allocated_vcpus).to eq(0)

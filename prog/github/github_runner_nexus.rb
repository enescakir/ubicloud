...
class Prog::Github::GithubRunnerNexus < Prog::Base
  ...
  def pick_vm
    ...
    alien_ratio = spill_enabled? ? spill_option.spill_ratio : 0
  ...
  # Spilling is active only when the installation has a spill option row that is
  # enabled. Disabling flips the flag without deleting the row, so its ratio and
  # limits survive to be re-enabled later.
  def spill_enabled?
    !!spill_option&.enabled
  end

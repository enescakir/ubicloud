...
class GithubRunner < Sequel::Model
  ...
  VM_LOG_VALUES_DS = DB[:vm]
    ...
    .select(
      ...
      Sequel.pg_jsonb_op(Sequel[:strand][:stack]).get_text(Sequel.pg_array(%w[0 ch_version], :text)).as(:ch_version),
    )
    .freeze
